...
WITH
  somedata -- explanation of what this does
...
AS
  ...
  ,foo.q
  ,foo.s
  ,SUM(foo.w) OVER (PARTITION BY foo.b, foo.a) AS w
  ,rank
FROM
  (
    SELECT
      ...
      ,ref.s
      ,
      -- another comment to explain something
      COUNT(COALESCE(bar.m, 1)) AS w
      ,RANK() OVER (PARTITION BY bar.b ORDER BY bar.a DESC) AS rank
      ,ROW_NUMBER() OVER (PARTITION BY bar.b, bar.m, bar.a ORDER BY bar.c DESC) AS row
    FROM
      database.schema.table bar
...
FROM
  (-- another explanatory comment
    SELECT
      CONVERT(DATE, (CONVERT(VARCHAR, C.date))) AS Date
     ,sd.b
     ,sd.c
     ,sd.d
     ,sd.e
     ...
     -- comment line 3
     ,cast((CASE
        WHEN C.counter = @SPECIALCODE
          THEN COALESCE(Pm.prest, Pm.weekpres2)
        ELSE C.counter_value
      END) as numeric(14,5)) / sd.w AS Minutes
     ,cast(CASE
        WHEN C.counter = @SPECIALCODE
          THEN COALESCE(Pm.prest, Pm.rprest, Pm.weekpres2)
        ELSE C.counter_value
    ...
    WHERE
      ...
      DATEADD(m, DATEDIFF(m, 0, CONVERT(DATE, (CONVERT(VARCHAR, C.bookdate)))), 0
      ) IN
      (
        SELECT
          sd.a
    ...
    UNION ALL
    -- simulates a standard row in a dimension table for example
    SELECT
      CONVERT(DATE, (CONVERT(VARCHAR, f.bookdate))) AS Date
     ,f.b
     ,f.c
     ,f.d
     ,f.e
